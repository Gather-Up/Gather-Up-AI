...
class ImageGenerationRequest(BaseModel):
    """Request schema for image generation with full SDXL customization"""
    
    # User prompt (will be enhanced by Llama)
    ...
    # Core SDXL parameters
    negative_prompt: Optional[str] = Field(
        default="blurry, low quality, distorted, deformed, ugly, bad anatomy, bad proportions, watermark, text, logo",
        description="What to avoid in the image"
    )
    
    width: int = Field(default=1024, ge=512, le=2048, description="Image width (SDXL optimal: 1024)")
    height: int = Field(default=1024, ge=512, le=2048, description="Image height (SDXL optimal: 1024)")
    
    # Generation quality parameters
    steps: int = Field(default=30, ge=1, le=150, description="Number of denoising steps (20-50 recommended)")
    cfg_scale: float = Field(default=7.0, ge=1.0, le=30.0, description="Classifier-free guidance scale (7-9 recommended)")
    
    # Sampler and scheduler
    sampler_name: SamplerName = Field(default=SamplerName.DPMPP_2M, description="Sampling method")
    scheduler: SchedulerName = Field(default=SchedulerName.KARRAS, description="Noise schedule")
